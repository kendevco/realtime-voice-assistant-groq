...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Image analysis failed', details: errorData }, { status: response.status });
    }
    ...
    return NextResponse.json({ error: 'Image analysis failed', details: error instanceof Error ? error.message : String(error) }, { status: 500 });
  }
}